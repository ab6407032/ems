...
def parse_and_load():
    # Parse the XML file
    #files = RouterLogFile.objects.filter(processed = False)
    files = RouterLogFile.objects.filter()
    for item in files:
        xml_path = item.file.path
        with open(xml_path, 'rb') as file:
            tree = ET.parse(xml_path)
    
        parsed_data = []

        # Find all 'mi' elements
        for mi in tree.xpath('//mi'):
            timestamp = mi.findtext('mts')
            period = mi.findtext('gp')
            counter_names = [mt.text.strip() for mt in mi.findall('mt')]
            mv = mi.find('.//mv')

            values = ['0'] * len(counter_names)  # Initialize all values to '0'
            if mv is not None:
                actual_values = [r.text.strip() if r.text is not None else '0' for r in mv.findall('r')]
                values[:len(actual_values)] = actual_values  # Replace initial values with actual ones up to the length of actual_values

            # Pair values with their counters and store in the database
...
# Usage
# parse_and_load('path_to_your_xml_file.xml')
